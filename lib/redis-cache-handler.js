...
const optionsSchema = {
    ...
    isValid: joi.func().default((q, cached) => _.isEqual(q, cached)),
    meta: joi.func().default(q => _.pick(q, 'limit', 'language', 'user', 'payload')),
    ...
    return function(query, reply) {
        ...
        const cacheResult = function(result) {
            return new P(function (resolve) {
                result.through(s => s.pipe(new RedisWriteStream(client, key)));
                client.multi()
                    .hset(key, 'selected', parseInt(result.selected))
                    .hset(key, 'fields', JSON.stringify(result.fields))
                    .hset(key, 'query', JSON.stringify(options.meta(query)))
                    .expire(key, options.expires)
                    .exec(err => {
        ...
        client.multi()
            ...
            .exec(function (err, replies) {
                ...
                // doesnt exist
                if (!replies[0] || !options.isValid(options.meta(query), JSON.parse(replies[3]))) return invokeHandler(query, reply);
